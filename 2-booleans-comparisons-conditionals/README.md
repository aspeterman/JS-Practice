...
3. Write a function `oldEnoughToDrink` that takes an `age` as an argument and
   returns `true` if the person with that age is old enough to drink.

4. There's an easy way to figure out how long a string is by adding `.length` to
...
5. Write a function `passwordLongEnough` that accepts a "password" as a
...
1. Write the following functions that each accept a single number as an
   argument:
...
2. A couple of other useful built-in mathematical functions are `Math.random`,
